...
class CommitMessage {
    ...
    private val MAX_LINE_LENGTH = 72 // https://stackoverflow.com/a/2120040/5138796

    companion object {
        val COMMIT_FIRST_LINE_FORMAT = Pattern.compile("^([a-z]+)(\\((.+)\\))?: (.+)")
        val COMMIT_CLOSES_FORMAT = Pattern.compile("Closes (.+)")
        ...
        fun parse(message: String): CommitMessage? {
            ...
            try {
                ...
                commitMessage.breakingChanges = stringBuilder.toString().trim { it <= ' ' }.replace("BREAKING CHANGE: ", "")
    ...
    constructor(
        changeType: ChangeType, changeScope: String, shortDescription: String, longDescription: String,
        breakingChanges: String, closedIssues: String, wrapText: Boolean, skipCI: Boolean
    ) {
    ...
    override fun toString(): String {
        ...
        if (StringUtils.isNotBlank(breakingChanges)) {
            var content = "BREAKING CHANGE: " + breakingChanges;
            builder
                .append(System.lineSeparator())
                .append(System.lineSeparator())
